...
namespace AsciiUtil
{
    public class ObjectPooler : MonoBehaviour
    {
        ...
        public PoolableObject GetActiveAnyRangeObject(PoolableObject targetObject, Vector3 originPosition, float range)
        {
            ...
            if (value == null)
            {
                Debug.LogError($"オブジェクトが見つからなかったよ ObjectKey: {targetObject.ObjectKey}");
            }
            return value;
        }
        ...
        public PoolableObject FindClosestObject(PoolableObject targetObject, Transform originTransform)
        {
            ...
            if (objects.Length <= 0)
            {
                Debug.LogError($"指定されたオブジェクトのプールにオブジェクトがないよ ObjectKey: {targetObject.ObjectKey}");
                return null;
            }
        ...
        public PoolableObject FindClosestObject(string targetObjectKey, Transform originTransform)
        {
            ...
            if (objects.Length <= 0)
            {
                Debug.LogError($"指定されたオブジェクトのプールにオブジェクトがないよ ObjectKey:{targetObjectKey}");
                return null;
            }
            ...
            if (value == null)
            {
                Debug.LogError($"オブジェクトが見つからなかったよ ObjectKey: {targetObjectKey}");
            }
            return value;
        }
